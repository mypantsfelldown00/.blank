...
def forecast_all(df: pd.DataFrame, periods: int = 30):
    ...
    if HAS_PROPHET:
        try:
            ...
            st.error(f"Enhanced Prophet error: {e}")
    ...
    if HAS_TF and HAS_SKLEARN:
        try:
            ...
            if sequence_length < 10:
                st.warning("Dataset too small for LSTM. Skipping LSTM forecasting.")
                forecasts['LSTM'] = None
            else:
            ...
            st.error(f"Enhanced LSTM error: {e}")
...
if run:
    ...
    with metrics_cols[1]: st.metric("Open", format_currency(last['Open'], currency_code))
    with metrics_cols[2]: st.metric("High", format_currency(last['High'], currency_code))
    with metrics_cols[3]: st.metric("Low", format_currency(last['Low'], currency_code))
    with metrics_cols[4]: st.metric("Close", format_currency(last['Close'], currency_code))
...
'''
    ...
    if not forecasts:
        st.info("No forecasts available. Ensure ML libraries are installed and data is sufficient.")
    else:
        for model_name, fc_df in forecasts.items():
            if fc_df is None or fc_df.empty:
                st.warning(f"{model_name} forecast not available or skipped.")
                continue
            st.markdown(f"**{model_name} Forecast**")
            st.dataframe(fc_df.tail(10).style.format({"yhat": lambda x: format_currency(x, currency_code)}))

            # Optional plot
            fig_fc = go.Figure()
            fig_fc.add_trace(go.Scatter(x=df['Date'], y=df['Close'], name='Actual Close', line=dict(color='white')))
            fig_fc.add_trace(go.Scatter(x=fc_df['Date'], y=fc_df['yhat'], name=f'{model_name} Forecast', line=dict(dash='dash', color='cyan')))
            fig_fc.update_layout(template='plotly_dark', title=f"{ticker} — {model_name} Forecast",
                                 xaxis_title="Date", yaxis_title=f"Price ({currency_code})")
...
with news_col:
    ...
    news_items = fetch_google_news(query, max_items=8)
    ...
    if not news_items:
        ...
        for item in news_items:
            ...
            <a class="card-link" href="{item['link']}" target="_blank">
                <div class="news-card">
                    {"<img class='news-img' src='" + item['image'] + "' />" if item['image'] else ""}
                    <div class="news-content">
